...
void	init_game_struct(t_game *game)
{
	...
	game->player.pos.x = round((game->player.pos.x + .5) * game->cell_size - PLAYER_SIZE / 2);
	game->player.pos.y = round((game->player.pos.y + .5) * game->cell_size - PLAYER_SIZE / 2);
	game->pplane = SCREEN_WIDTH / (2.0 * tan(FOV * DEG_TO_RAD / 2.0));
	game->camera_pos.x = round(game->player.pos.x) + PLAYER_SIZE / 2 - 1;
	game->camera_pos.y = round(game->player.pos.y) + PLAYER_SIZE / 2 - 1;
