...
float	vertical_intersect(t_game *game, float angle)
{
	...
	if (angle == M_PI / 2 || angle == 3 * M_PI / 2)
		angle += 0.0001;
	//increase_x = game->cell_size;
	increase_y = game->cell_size * tan(angle); //how much y changes when x changes by 1 (or -1) block_size
	...
	//printf ("game->camera.x %d, floor(game->camera.x / (game->cell_size)) %f\n", game->camera.x, floor(game->camera.x / (game->cell_size)));
