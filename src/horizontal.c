...
float	horiz_intersect(t_game *game)
{
	...
	increase_x = game->cell_size / tan(game->ray->current_angle);
	...
	game->ray->intersect.x = (game->camera.pos.x - X_START) - (game->ray->intersect.y - game->camera.pos.y) / tan(game->ray->current_angle);
